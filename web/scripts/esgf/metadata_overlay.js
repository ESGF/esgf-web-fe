...
function metadata_report(id,title,metadatafilename,metadatafileformat)
{
	...
	jQuery.ajax({
		...
		data: 'facet=true',
		type: 'GET',
		success: function(data) {echo(data,id);},
	...
	//here we obtain the result (in json format) from the metadata file
	//it is an ajax call to the server housing the metadata file
	...
	var url = url;
	var west_degrees = -90;//west_degrees;
	var east_degrees = -90;//east_degrees;
	var north_degrees = 36;//north_degrees;
	var south_degrees = 36;//south_degrees;
	var datetime_start = datetime_start;
	var datetime_stop = datetime_stop;
	
	
	$('div#metadata_summary_dataset').replaceWith('<div class="addedMetadataTitle">' + 'Dataset: ' + title + '</div><p>&lt;insert constraints here&gt;</p>');
...
function echo(data,id)
{
	alert('echoing ' + data + ' ' + id);
	alert(data.menu.id);
}
